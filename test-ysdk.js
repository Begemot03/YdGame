        ...
        init: () => {
            ...
            return new Promise((resolve) => {
                ...
                resolve({
                    ...
                    // Объект player (cloud data / stats)
                    player: {
                        // Локальное «хранилище»
                        __localData: {},
                        __localStats: {},
                        ...
                        getData: async function (keys) {
                            console.log("[TestYSDK] player.getData() заглушка, keys=", keys);
                            console.log(this.__localData);
                            if (!keys) {
                                // вернуть все
                        ...
                        setStats: async function (statsObj) {
                            console.log("[TestYSDK] player.setStats() заглушка, stats=", statsObj);
                        ...
                        getStats: async function (keys) {
                            console.log("[TestYSDK] player.getStats() заглушка, keys=", keys);
                            console.log(this.__localStats);
                            if (!keys) {
                        ...
                        incrementStats: async function (increments) {
                            console.log("[TestYSDK] player.incrementStats() заглушка, increments=", increments);
                            console.log(this.__localStats);
                            let updated = {};
                    ...
                    getLeaderboards: async () => {
                        ...
                        return {
                            ...
                            setLeaderboardScore: async (lbName, score) => {
                                console.log(
                                    "[TestYSDK] setLeaderboardScore() заглушка, name=", lbName, "score=", score
                                );
                                // Можно хранить в каком-то локальном объекте, если хотите
                            },
                            // Получить рекорд игрока
                            getLeaderboardPlayerEntry: async (lbName) => {
                                console.log("[TestYSDK] getLeaderboardPlayerEntry() заглушка, name=", lbName);
                            ...
                            getLeaderboardEntries: async (lbName, {quantityTop, includeUser, quantityAround}) => {
                                console.log("[TestYSDK] getLeaderboardEntries() заглушка, lbName=", lbName);
                                console.log("  quantityTop=", quantityTop, " includeUser=", includeUser, " quantityAround=", quantityAround);
                    ...
                    getPayments: async (options) => {
                        console.log("[TestYSDK] getPayments() заглушка, signed=", options?.signed);
                        this.__paymentsInited = true;
                        return {
                            purchase: async ({ id, developerPayload }) => {
                                console.log("[TestYSDK] purchase() заглушка, id=", id, "payload=", developerPayload);
                            ...
                            consumePurchase: async (token) => {
                                console.log("[TestYSDK] consumePurchase() заглушка, token=", token);
                            },
                            getPurchases: async () => {

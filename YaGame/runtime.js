...
(function () {
		...
		(async function () {
			if (typeof YaGames !== "undefined") {
				try {
					...
					console.log("[YandexGames plugin] SDK инициализирован");
					window.flags = await window.ysdk.getFlags();
					console.log("[YandexGames plugin] Флаги получены");
					instanceProto.player = await window.ysdk.getPlayer({
						scopes: false,
					});
					console.log("----------player", instanceProto.player);
					instanceProto.playerIsAuth = instanceProto.player.getMode() != "lite";

					if (window.ysdk.features.LoadingAPI?.ready) {
						window.ysdk.features.LoadingAPI.ready();
						console.log("[YandexGames plugin] SDK готов к запуску");
					}
				} catch (err) {
					console.error("[YandexGames plugin] Ошибка инициализации SDK:", err);
				}
			} else {
				console.error(
					"[YandexGames plugin] YaGames SDK не найден. Убедитесь, что <script> подключён!"
				);
			}
		})();
	};
	...
	Acts.prototype.AuthPlayer = async function () {
		if (!this.isPlayerFullDataGetted) {
	...
	Exps.prototype.PlayerId = function (ret) {
		if (this.player && typeof this.player.getUniqueID === "function") {
			ret.set_string(this.player.getUniqueID());
		} else {
			ret.set_string("");
		}
	};
